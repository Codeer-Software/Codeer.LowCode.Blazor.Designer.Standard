...
| `Submit(List<Module> simultaneousWriteModules)` | bool? | `this`（自モジュール）と引数リストの全モジュールを**1トランザクションで同時保存**する。戻り値は `Submit()` と同じ `bool?`。`this` も一緒に保存されるので引数リストに入れる必要はない。失敗時は全ロールバック |
