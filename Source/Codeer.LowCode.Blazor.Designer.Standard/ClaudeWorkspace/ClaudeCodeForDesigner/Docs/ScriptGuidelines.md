...
## 大前提: スクリプトは「インタプリタ実行」で、コンパイル済み C# より桁違いに遅い

CLB のスクリプトは C# ライクな構文だが、**ビルドして機械語（IL）になるわけではない**。実行時に構文木を解釈しながら1ノードずつ評価し、フィールド・メソッドへのアクセスは毎回リフレクションで解決する**ツリーウォーク型インタプリタ**（WebAssembly でも動かすためこの方式）。そのため:

- **1 命令あたりのオーバーヘッドが通常の C# より大きい**（体感で桁違い）。同じ式でも、ループで N 回評価すればインタプリタのコストが N 倍かかる。
- **重いのは I/O（DB / 通信）だけではない**。`Field.Value` の読み書き・メソッド呼び出し・プロパティアクセスといった「普通の C# なら一瞬」の操作も、大量に回すと遅い。
...
大きな UI 更新や通信を伴う処理では、あわせて**再描画の抑止**と**ローディング表示の集約**も行う（後述の該当節）。まとめると、スクリプトで意識するのは次の3つ:
...
## 通信をまとめる: 独立した複数の取得は BatchSearcher で 1 往復に

複数の `ModuleSearcher` を別々に `Execute()` すると、その回数だけクライアント→サーバ→DB の往復が発生する（WASM では特に往復コストが重い）。取得が互いに独立なら **`BatchSearcher.Execute(...)` で 1 往復に束ねる**:

```csharp
// NG: 3 往復（Execute を個別に呼ぶ）
var orderSearch = new ModuleSearcher<Order>();
orderSearch.AddEquals(e => e.Status.Value, "Open");
var orders    = orderSearch.Execute();
var customers = new ModuleSearcher<Customer>().Execute();
var products  = new ModuleSearcher<Product>().Execute();

// OK: 1 往復（BatchSearcher で束ねる）
var os = new ModuleSearcher<Order>();
os.AddEquals(e => e.Status.Value, "Open");
var res = BatchSearcher.Execute(os, new ModuleSearcher<Customer>(), new ModuleSearcher<Product>());
var orders    = res.GetAt(0);   // 渡した順（0 始まり）。res.GetBy(os) でも取れる
var customers = res.GetAt(1);
var products  = res.GetAt(2);
```
...
全メソッドの正確なシグネチャと使用例は [ClaudeCodeForDesigner/_script_catalog.md](../ClaudeCodeForDesigner/_script_catalog.md) の Excel セクションを参照。
...
単一引数ラムダ (`x => x.Value`) は `ModuleSearcher` の条件メソッドでのみ使える ([ClaudeCodeForDesigner/_specs/Scripts.md](ClaudeCodeForDesigner/_specs/Scripts.md) のラムダ式セクション参照)。
