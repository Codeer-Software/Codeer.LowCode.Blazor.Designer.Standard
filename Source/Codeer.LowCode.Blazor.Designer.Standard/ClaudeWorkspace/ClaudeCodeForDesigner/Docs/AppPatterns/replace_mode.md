...
- 子モジュールに権限条件を付けている場合、洗い替えの削除も権限に従う。
  **ユーザーの書込権限 (`UserWriteCondition`) が無いユーザーは洗い替え保存自体が失敗**し、
  **書き込みできる行の条件 (`DataWriteCondition`) の条件外の行は削除されずに残る** (エラーにはならない)。
  後者は「残った条件外の行 + 入れ直した現在の行」が並ぶ状態になるため、
  権限条件付きモジュールを完全洗い替えする設計は避けるか、条件外の行が残る前提で組む
...
- [ListField](ClaudeCodeForDesigner/_field_catalog.md)
