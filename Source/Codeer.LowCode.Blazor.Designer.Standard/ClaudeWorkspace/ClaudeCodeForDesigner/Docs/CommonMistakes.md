...
詳細は [ViewEditToggleButtonField](ClaudeCodeForDesigner/_field_catalog.md) の「重要な副作用」セクション参照。
...
詳細は [DatabaseGuidelines.md](DatabaseGuidelines.md) と [IdField](ClaudeCodeForDesigner/_field_catalog.md) を参照。
...
詳細は [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) の ListPageDesign セクションを参照。
...
**標準パターン (`ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Product.mod.json` 等の詳細レイアウトを参考):**
...
詳細は [AnchorTagField](ClaudeCodeForDesigner/_field_catalog.md) を参照。
...
詳細は [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) の `OtherPageModuleDesigns` セクションを参照。
...
参考: `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Order.mod.json` の `Details` (ListField) + `OrderDetail.mod.json` を参照。
...
詳細は [FileField](ClaudeCodeForDesigner/_field_catalog.md) の「サーバ側設定が必須」セクション参照。
...
詳細は [ClaudeCodeForDesigner/_specs/Layouts.md](ClaudeCodeForDesigner/_specs/Layouts.md) の `GridRow` プロパティ表参照。
...
- 実装サンプル: `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/CascadeSearch.mod.json` + `CascadeSearch.mod.cs` (サイドバー「検索/Select連動」)
...
- `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Order.mod.json` … `Details` フィールドは **`ListFieldDesign`** (`DetailListFieldDesign` ではない)
- `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/OrderDetail.mod.json` … 親 FK `OrderId` は **`IdFieldDesign`** (`IsManualInput:false`)、明細列は `ListLayouts[""].Elements` に Product/Quantity/UnitPrice/Subtotal を定義

**鉄則:** 迷ったら **`ListField`**。`DetailListField` を選ぶのは「各行を 1 枚のフォーム/カードにしたい」と明確に判断したときだけ。均一な明細表に `DetailListField` を使うと、行ごとにラベルが重複し、子モジュールに表示用ラベルとカード用 DetailLayout を足す羽目になって冗長。**パターンドキュメントやサンプルの Description を鵜呑みにせず、実装に迷ったら必ず `Order.mod.json` / `OrderDetail.mod.json` を開いて型を確認する。** 関連: [AppPatterns/header_detail.md](AppPatterns/header_detail.md) / [ListField](ClaudeCodeForDesigner/_field_catalog.md) / [DetailListField](ClaudeCodeForDesigner/_field_catalog.md) の「どちらを選ぶか」。
...
**正しい書き方 — 文字列補間の書式を使う** (内部で `string.Format` に委譲され動作する。[ClaudeCodeForDesigner/_specs/Scripts.md](ClaudeCodeForDesigner/_specs/Scripts.md) の文字列補間):
