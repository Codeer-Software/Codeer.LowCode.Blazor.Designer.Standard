...
詳細は [temporary/_specs/PageFrame.md](temporary/_specs/PageFrame.md) の ListPageDesign セクションを参照。
...
**標準パターン (`temporary/_samples/PatternShowcase/Modules/Product.mod.json` 等の詳細レイアウトを参考):**
...
詳細は [temporary/_specs/PageFrame.md](temporary/_specs/PageFrame.md) の `OtherPageModuleDesigns` セクションを参照。
...
参考: `temporary/_samples/PatternShowcase/Modules/Order.mod.json` の `Details` (ListField) + `OrderDetail.mod.json` を参照。
...
詳細は [temporary/_specs/Layouts.md](temporary/_specs/Layouts.md) の `GridRow` プロパティ表参照。
...
- 実装サンプル: `temporary/_samples/PatternShowcase/Modules/CascadeSearch.mod.json` + `CascadeSearch.mod.cs` (サイドバー「検索/Select連動」)
...
- `temporary/_samples/PatternShowcase/Modules/Order.mod.json` … `Details` フィールドは **`ListFieldDesign`** (`DetailListFieldDesign` ではない)
- `temporary/_samples/PatternShowcase/Modules/OrderDetail.mod.json` … 親 FK `OrderId` は **`IdFieldDesign`** (`IsManualInput:false`)、明細列は `ListLayouts[""].Elements` に Product/Quantity/UnitPrice/Subtotal を定義
...
- 複数の着地フレームを作るのは **PC/スマホで出し分けたいときだけ**。その場合は複数を `true` にして `TargetDevice` / `WidthFrom` / `Priority` で振り分ける ([temporary/_specs/PageFrame.md](temporary/_specs/PageFrame.md) の「画面幅・デバイスでアプリケーションルートを切り替える」)。
- 正典: `temporary/_samples/PatternShowcaseAuth/PageFrames/Main.frm.json` (`true`) + `AdminFrame.frm.json` (`false`)。
- 詳細は [temporary/_specs/PageFrame.md](temporary/_specs/PageFrame.md) の冒頭ルールを参照。
...
**正しい書き方 — 文字列補間の書式を使う** (内部で `string.Format` に委譲され動作する。[temporary/_specs/Scripts.md](temporary/_specs/Scripts.md) の文字列補間):
...
**正解:**
- **集計・件数は DB 側で 1 回** — `QueryField` / `ExecuteSqlField` で `GROUP BY` する ([temporary/_specs/QueryAndSql.md](temporary/_specs/QueryAndSql.md))。例: `SELECT status, COUNT(*) FROM repair_order GROUP BY status` の 1 クエリ。
- **画面上の明細行の集計**は DB に行かず `Rows` を使う (#4 参照)。
- **複数の独立した取得**は `BatchSearcher` で 1 往復に束ねる ([temporary/_specs/Scripts.md](temporary/_specs/Scripts.md) の BatchSearcher)。
