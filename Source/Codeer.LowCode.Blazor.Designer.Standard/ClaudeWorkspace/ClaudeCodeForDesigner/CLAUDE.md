...
> **注意: この `ClaudeCodeForDesigner/` フォルダはデザイナが丸ごと再生成する（手で編集・追加したものは消える）。** ワークスペースの展開・更新（Tools > Claude Code Workspace / `claude-workspace` / フックの自動更新）のたびに削除して作り直される。恒久的なメモはワークスペース直下の `Project.md`（プロジェクト共有）か `LocalEnvironment.md`（マシン固有）へ。
...
3. **既存をコピーして改変する（ゼロから JSON を組まない）**。
   ...
   - レイアウトは包み側の型をコピーする: 詳細は `_defaults/DetailLayoutDesign.json`、一覧は `ListLayoutDesign.json`、検索は `SearchLayoutDesign.json`。空行 1 本・空セル 1 個・検索の折りたたみといった初期値がデザイナの新規作成と同じ状態で入っている。行を足すときは `GridRow.json`（空の 4 列）をコピーする。**`Rows: []` / `Elements: []` の空レイアウトは書かない**（デザイナでドロップ先が無くなり編集できない）。
...
**設定ファイルの実例:**
- [ClaudeCodeForDesigner/_samples/PatternShowcase/](ClaudeCodeForDesigner/_samples/PatternShowcase/) - **アプリ作成パターンの全実装サンプル** (データ操作 / 検索 / 一覧 / ダイアログ / レイアウト / 入力UX / 出力 / 別フレーム、および認証・権限・承認 (ApprovalFlowField)。[Docs/AppPatterns/](Docs/AppPatterns/) と対応、JSON / SQL / C# テキスト同梱。Cookie 認証付き)

機能を実装するときは、まず [Docs/AppPatterns/](Docs/AppPatterns/) で該当パターンを確認し、そのパターン名の対応モジュール (各パターン末尾の「標準パターン集の対応」セクション参照) を [ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/](ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/) から開いて、JSON / SQL / C# スクリプトの実物を参考にすること。
...
> **DB に接続する点に注意 (重要)**: このチェックはプロジェクトの接続文字列で DB に接続する。`designer.settings.Development.json` が**本番 DB を指している可能性**もあるため、接続先が不明なプロジェクトに対して実行する前は接続先を確認すること ([ClaudeCodeForDesigner/_specs/ProjectSettings.md](ClaudeCodeForDesigner/_specs/ProjectSettings.md) 参照)。DB が起動していない/到達不能なデータソースは「データソースが存在しません」等として報告される (クラッシュはしない)。

### デザイナ exe のパス

[LocalEnvironment.md](LocalEnvironment.md) の `DesignerExePath:` 行にある (このワークスペースを展開したデザイナ自身のパスを claude-workspace が自動記録している)。無い・実在しない・CLI が動かない場合は、**ディスクを探索して勝手に直さず、ユーザーに正しいパスを確認する**。ユーザーから新パスを指示されたら `LocalEnvironment.md` の `DesignerExePath:` 行と `.claude/settings.local.json` 内の旧パス全出現を書き換えてよい (手順の詳細はルートの CLAUDE.md)。デザイナのメニュー **Tools > Claude Code Workspace** の再実行を案内するのでもよい。
...
- 標準テンプレート (空のプロジェクト / 空のプロジェクト（認証なし WPF/WinForms） / 入門サンプル / 標準パターン集 / 在庫管理 / SFA / プロジェクト管理) はローカル SQLite なので最初から `true`
...
## リネーム CLI (フィールド / モジュール / ページフレーム / レイアウト / enum の名前変更)

フィールド・モジュール・ページフレーム・レイアウト・enum (定義とメンバー) の名前を変えるときは、**手作業で JSON をテキスト置換しない**。参照は JSON のプロパティだけでなく、スクリプト (C# 構文解析が必要)・リンク越しの参照 (`○○Link.対象名`)・他モジュールの条件/レイアウトなど広範に散在し、テキスト置換では**必ず取りこぼす／別物まで巻き込む**。
...
"<デザイナexeのパス>" rename-enum        "<プロジェクトのルートフォルダ>" --from <旧名> --to <新名> --out "<結果JSON>"
"<デザイナexeのパス>" rename-enum-member "<プロジェクトのルートフォルダ>" --enum <enum名> --from <旧名> --to <新名> --out "<結果JSON>"
```

- 第2引数 … `app.clprj` があるプロジェクトのルートフォルダ
- `--module` … 対象モジュール名 (`rename-field` / `rename-layout` で必須)
- `--layout-type` … `Detail` / `List` / `Search` (`rename-layout` で必須)
- `--enum` … 所属 enum 名 (`rename-enum-member` で必須)
- `--from` / `--to` … 旧名 / 新名
...
- **サイドカー / ファイル自体の改名**:
  ...
  - `rename-enum`: `{enum}.enum.json` 本体を改名し、`SelectField.EnumName`・スクリプトの enum 参照を張り替え
  - `rename-enum-member`: スクリプトのメンバー参照を張り替え。**`Value` 省略のメンバーは旧 Name が `Value` に固定される** (DB 内の既存保存値を守るため。表示も旧値のままになるのは仕様)
...
## AI 用生成リファレンス一式 (ClaudeCodeForDesigner/) と ai-refresh

この `ClaudeCodeForDesigner/` フォルダには、配布ドキュメント (この CLAUDE.md と Docs/) に加えて、**インストール済みデザイナ (拡張ライブラリ・このプロジェクトの独自フィールド込み) から動的に生成したリファレンス一式**が置かれる。すべて生成物 (フォルダごと gitignore 済み・手で編集しない) で、正本はデザイナ側にあるため**常にインストール済みバージョンと一致した正**になる:
...
```
"<デザイナexeのパス>" ai-refresh "<プロジェクトのルートフォルダ>" --out-dir "ClaudeCodeForDesigner"
```
...
```
"<デザイナexeのパス>" field-catalog "<プロジェクトのルートフォルダ>" --out "ClaudeCodeForDesigner/_field_catalog.md"
```
...
**出力先は `ClaudeCodeForDesigner/`**。`ClaudeCodeForDesigner/_field_catalog.md` は毎回作り直す**生成物**（フォルダごと gitignore 済み。ワークスペース更新時にフォルダごと作り直される）。

### 自動再生成（ビルド検知）

`.claude/settings.local.json`（claude-workspace が exe パス入りで自動生成）の **SessionStart / UserPromptSubmit フック**が `.claude/refresh-ai-workspace.ps1` を呼び、次の仕組みで**必要なときだけ** `claude-workspace` を叩き直し、`ClaudeCodeForDesigner/`（Docs も本カタログ含む生成リファレンスも）を丸ごと最新化する:

- デザイナ exe は独自フィールドライブラリを参照してビルドされるため、フィールドの追加/削除は**再ビルド = exe と同フォルダ DLL のタイムスタンプ更新**として現れる。
- フックはそのタイムスタンプ最大値を `ClaudeCodeForDesigner/_ai_refresh.stamp`（ai-refresh 成功時に deploy が記録）と照合し、**前回と同じならスキップ・変化していれば丸ごと入れ替え**る。フォルダごと作り直すので、**削除されたフィールドやドキュメントも消える**。
...
> フィールド型を使うときは、まず `ClaudeCodeForDesigner/_field_catalog.md`（このプロジェクトで実際に使える型の一覧・構造）を確認し、解説が必要なら `Docs/Fields/` を読む。独自フィールドは `Docs/Fields/` に無いのでカタログが唯一の情報源になる。
...
```
"<デザイナexeのパス>" script-catalog "<プロジェクトのルートフォルダ>" --out "ClaudeCodeForDesigner/_script_catalog.md"
```
...
field-catalog と同じ扱い: **出力先は `ClaudeCodeForDesigner/_script_catalog.md`**（毎回作り直す生成物・gitignore 済み）。`.claude/refresh-ai-workspace.ps1` フックが `ai-refresh` で他の生成物と同時に「デザイナ再ビルド検知時だけ」再生成する。

> 組み込み外のサービス (Excel / WebApi / Toaster / Mail 等) をスクリプトで使うときは、`ClaudeCodeForDesigner/_script_catalog.md` でこの環境に登録されているか・正確なシグネチャ・使用例を確認する。カタログに無いサービス・型は使えない。独自のサービス/型を追加する方法は [ClaudeCodeForDesigner/_specs/ScriptExtensions.md](ClaudeCodeForDesigner/_specs/ScriptExtensions.md) を参照。
...
| ドキュメント | 内容 |
|---|---|
| [ClaudeCodeForDesigner/_specs/ModuleDesign.md](ClaudeCodeForDesigner/_specs/ModuleDesign.md) | モジュール定義 (*.mod.json) の全体構造、CRUD権限、権限条件 |
| [ClaudeCodeForDesigner/_specs/Layouts.md](ClaudeCodeForDesigner/_specs/Layouts.md) | レイアウト（Detail/List/Search, Grid/Tab/Canvas, FieldLayout） |
| [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) | ページフレーム (*.frm.json) ナビゲーション構造 |
| [ClaudeCodeForDesigner/_specs/SearchConditions.md](ClaudeCodeForDesigner/_specs/SearchConditions.md) | 検索条件 (FieldValueMatch, FieldVariableMatch, MultiMatch) |
| [ClaudeCodeForDesigner/_specs/QueryAndSql.md](ClaudeCodeForDesigner/_specs/QueryAndSql.md) | クエリとSQL実行の総合ガイド（QueryField / ExecuteSqlField） |
...
業務アプリを CLB で組むときに繰り返し登場する「型」を解説する。**機能を実装する前にまず該当パターンを引き、対応する PatternShowcase のモジュールを参考実装として読むこと**。

| ドキュメント | 内容 |
|---|---|
| [Docs/AppPatterns/patterns.md](Docs/AppPatterns/patterns.md) | **全パターンのインデックス**。A〜J の 10 カテゴリ。ここから引く |
| [Docs/AppPatterns/auth_patterns.md](Docs/AppPatterns/auth_patterns.md) | 認証・権限・承認パターンの入口 (`PatternShowcase` の認証まわりの対応) |

各パターンの末尾には **「標準パターン集の対応」** セクションがあり、`ClaudeCodeForDesigner/_samples/PatternShowcase/` のどのモジュールが実装サンプルかを示している。例: 「マスタ参照 (多対1)」パターン → `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Shop.mod.json` + `ShopType.mod.json` を読む。
...
各フィールド型（組み込み + このプロジェクトの独自フィールド）の TypeFullName・プロパティ・既定値・候補・`## Design`/`## Script`/`## CSS` ドキュメントは、**自動生成カタログ [ClaudeCodeForDesigner/_field_catalog.md](ClaudeCodeForDesigner/_field_catalog.md) に集約**されている（デザイナ exe の `field-catalog` サブコマンドが生成、SessionStart フックで最新化。前述「フィールド型カタログ CLI」参照）。フィールド型を使うときはまずこのカタログで型名を grep して構造・例を確認する。**独自フィールド（ProCode / 拡張ライブラリ）はここにしか載らない**ので、独自フィールドを含むプロジェクトでは唯一の情報源になる。

**共通ドキュメント（`_` プレフィックス、カタログには含まれない共通基底）:**
- [ClaudeCodeForDesigner/_specs/_FieldCommon.md](ClaudeCodeForDesigner/_specs/_FieldCommon.md) - 共通基底プロパティ（FieldDesignBase, ValueFieldDesignBase, DbValueFieldDesignBase, ListFieldDesignBase）
- [ClaudeCodeForDesigner/_specs/_ScriptApi.md](ClaudeCodeForDesigner/_specs/_ScriptApi.md) - フィールド共通スクリプトAPI（全フィールド共通・値フィールド共通のプロパティ/メソッド）
...
├── Enums/                            # デザイン enum 定義（オプション）
│   └── *.enum.json                   # 列挙型定義 (JSON)。詳細: _specs/DesignEnums.md
...
15. **IsViewOnly はレイアウト要素のプロパティ** - フィールド定義（Fields配列）ではなく、FieldLayoutDesign / GridLayoutDesign / ListElement に設定する。詳細は [ClaudeCodeForDesigner/_specs/Layouts.md](ClaudeCodeForDesigner/_specs/Layouts.md) の IsViewOnly セクション参照
16. **一覧ページのソートは PageFrame で設定** - モジュールの `ListPageFieldDesign` ではなく、PageFrame の `ListPageDesign.ListFieldDesign.SearchCondition.SortConditions` で設定する。詳細は [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) のソート設定の優先順位を参照
17. **LinkFieldNames** - 他モジュールのフィールドを表示するには `LinkFieldNames` にパスを追加し、レイアウトでも参照が必要。詳細は [ClaudeCodeForDesigner/_specs/ModuleDesign.md](ClaudeCodeForDesigner/_specs/ModuleDesign.md) の LinkFieldNames の詳細を参照
...
23. **サイドバーの階層メニュー** - PageFrame の `Title` に `/` 区切りで文字列を指定すると階層メニューが自動生成される（例: `"マスタ/取引先"` → 「マスタ」グループ配下に「取引先」リンク）。詳細は [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) の Title による階層メニュー を参照
24. **カラム枠線は BorderStyle で設定** - `Border` プロパティ（"None"/"All"等）は非推奨。`BorderStyle` の太さ（`Left`/`Top`/`Right`/`Bottom`）と色（`LeftColor`等）の両方を設定すること。太さを指定しないと線は表示されない。隣接セルとの重複回避ルールあり。詳細は [ClaudeCodeForDesigner/_specs/BorderStyleGuide.md](ClaudeCodeForDesigner/_specs/BorderStyleGuide.md) を参照
...
31. **サイドバー/ヘッダーをモジュール化できる** - `SideBarDesign.ModuleName` / `HeaderDesign.ModuleName` に表示専用モジュールを指定すると、標準UIの代わりにそのモジュールの `DetailLayouts[""]` が描画される。Home/Links/Logout は出なくなるので必要なら自前実装する。詳細は [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) の ModuleName セクションを参照
...
36. **一覧→詳細フローの詳細画面に戻るボタンを置く** - `AnchorTagFieldDesign` の `Target: "HistoryBack"` + `Icon: "bi bi-arrow-left-circle-fill"` + `FontSize: 30` を DetailLayout の 1 行目に配置。`ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Product.mod.json` 等の詳細レイアウト参考。詳細は [Docs/CommonMistakes.md](Docs/CommonMistakes.md) の #36 を参照
...
62. **式メソッド `=>` と複数引数ラムダ `(a,b)=>...` は使えない** - `string F() => "x";` (expression-bodied method) は不可、ブロック体 `string F() { return "x"; }` で書く。`list.Sort((a,b) => ...)` も不可、手動ソートする。単一引数ラムダ `x => x.Value` は `ModuleSearcher` の条件メソッド内のみ可。詳細は [ClaudeCodeForDesigner/_specs/Scripts.md](ClaudeCodeForDesigner/_specs/Scripts.md) のラムダ式セクション + [Docs/ScriptGuidelines.md](Docs/ScriptGuidelines.md) 参照
...
69. **承認・ワークフローは求められない限り作らない／非認証アプリに承認を足さない** - 既定は CRUD（ヘッダ＋明細＋合計等）に留め、承認ボタン等を勝手に足さない・「推奨」もしない。承認は本来 承認者・段階・差し戻し・履歴・権限を伴う重い機能で、認証（ログイン）が前提。非認証アプリでは申請者と承認者を区別できず無意味。承認が必要なら認証前提で Extras の `ApprovalFlowField` として正式に作る（承認モジュール群・承認待ち/承認状況一覧・経路マスタはデザイナの `approval-setup` CLI が生成し、申請書側は ApprovalFlowField を 1 つ置いて `OnBuildRoute` と編集ロックの DataWriteCondition を書くだけ。実装例 `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/ExpenseRequest.mod.json` / [Docs/AppPatterns/auth_workflow.md](Docs/AppPatterns/auth_workflow.md)）
70. **明細表 (ヘッダ＋明細) は `ListField`。`DetailListField` ではない (致命的・最頻出の誤り)** - 「注文＋明細」「請求書＋明細」「経費精算＋明細」のように日付・科目・金額… を**列の揃った表**で 1 行ずつ並べる明細は **`ListFieldDesign`** を使い、**列定義は子 (行) モジュール側の `ListLayouts[""].Elements`** に書く。子の親 FK は `IdFieldDesign` (`IsManualInput:false`)。`DetailListFieldDesign` は名前に「明細 (Detail)」が入っているので「明細表＝DetailListField」と短絡しがちだが**意味は逆** — "Detail" は「各行を DetailLayout (フォーム/カード) で描く」の意味で、業務の「明細行」ではない。`DetailListField`/`TileListField` を選ぶのは「1 レコード＝1 枚のフォーム/カード」にしたいと明確に判断したときだけ (その場合は子を `IsBordered:true` でカード化必須)。**迷ったら `ListField`。** 実装に迷ったら正典 `ClaudeCodeForDesigner/_samples/PatternShowcase/Modules/Order.mod.json` (`Details`＝`ListFieldDesign`) と `OrderDetail.mod.json` (`OrderId`＝`IdFieldDesign`、列は `ListLayouts`) を必ず開いて型を確認する。詳細は [Docs/CommonMistakes.md](Docs/CommonMistakes.md) の #53 / [Docs/AppPatterns/header_detail.md](Docs/AppPatterns/header_detail.md) を参照
71. **PageFrame は最低 1 つ `IsApplicationRoot: true` (ルート URL の着地フレーム) を作る (絶対常識)** - `IsApplicationRoot` の C# 既定は `false`。新規 PageFrame を起こして着地フレーム (通常 `Main`) に `true` を立て忘れると、プロジェクトに application root が 0 件になり、ルート URL (`/`) を開いたとき CLB が**非 application-root のフレームにフォールバック**する。`UserReadCondition` で権限ゲートした管理フレーム (`AdminFrame` 等) が既定ランディングに選ばれる事故になる (admin だけ管理画面がホームになる等)。**着地フレームは `true`、補助フレームは `false`**。複数の `true` は PC/スマホ出し分け (`TargetDevice`/`WidthFrom`/`Priority`) のときだけ。単一フレームのプロジェクトでもその 1 枚を必ず `true` にする (空テンプレ複製時に踏みやすい)。正典: `ClaudeCodeForDesigner/_samples/PatternShowcase/PageFrames/Main.frm.json` (`true`) + `AdminFrame.frm.json` (`false`)。詳細は [Docs/CommonMistakes.md](Docs/CommonMistakes.md) の #54 / [ClaudeCodeForDesigner/_specs/PageFrame.md](ClaudeCodeForDesigner/_specs/PageFrame.md) 冒頭ルールを参照
72. **スクリプトの数値書式・ゼロ埋めは `ToString("D3")` ではなく文字列補間 `$"{n:000}"` を使う (実行時エラー・デバッグ困難)** - CLB のスクリプトは数値を `decimal` に統一して計算するため、`(n).ToString("D3")` のような整数専用書式は**実行時に `Format specifier was invalid.` で失敗**する (designcheck は緑で素通り)。`$"{cnt + 1:000}"` / `$"{date:yyMMdd}"` なら `string.Format` 経由で動く。厄介なのは、例外でイベントハンドラが**途中停止**し後続 (合計再計算・ボタン出し分け等) が走らず、一見**別の不具合**に見える点。書式まわりで挙動が変なときはまず `ToString(書式)` を疑う。詳細は [Docs/CommonMistakes.md](Docs/CommonMistakes.md) の #55 / [ClaudeCodeForDesigner/_specs/Scripts.md](ClaudeCodeForDesigner/_specs/Scripts.md) の文字列補間を参照
73. **DB の集計・件数を `ModuleSearcher` のループで回さない (N+1)** - 状態別件数などを状態の数だけ `ModuleSearcher.Execute()` でループすると、WASM のクライアント→サーバ→DB 往復が件数分発生する。**集計・件数は `QueryField`/`ExecuteSqlField` で `GROUP BY` の 1 クエリ**にまとめる。画面上の明細集計は `Rows`、複数の独立取得は `BatchSearcher` で 1 往復に束ねる。「この処理は問い合わせ何回か」を実装前に見積もり、ループ内で I/O を回したら赤信号。詳細は [Docs/CommonMistakes.md](Docs/CommonMistakes.md) の #56 / [ClaudeCodeForDesigner/_specs/QueryAndSql.md](ClaudeCodeForDesigner/_specs/QueryAndSql.md) を参照
